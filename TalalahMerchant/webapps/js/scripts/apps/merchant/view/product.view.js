...
define(function(require,exports,module){
	...
	var content = talalah.com.client.app.page.content;
	...
	var ProductImgView = Backbone.View.extend({
		render		:	function(){
			...
			if($.isEmptyObject(data.merchant.merchantCode)===false){
				...
				var pId		= data.id;
				var img		= data.img;
				this.$el.find('#productImg').attr('src',content+ '/' + mcc + '/' + mcId + '/' + pId + '/' + img);
			}
			else{
				this.$el.find('#productImg').attr('src','/TalalahMerchant/content/util/img/default.png');
			}
			return this.$el;
		},
		events : {
		...
		render		:	function(){
			if($.isEmptyObject(this.model)===false){
				...
				if(data.tag !== null){
					...
					this.$el.html(this.template(data));
					this.$el.find('#img').attr('src',content+'/'+mcc+'/'+mcId+'/'+pId+'/'+pId+'/'+name);

					this.$el.find('#editable').show();
					this.$el.find('#uploadBtn').hide();
				...
				this.$el.find('#img').attr('src','/TalalahMerchant/content/util/img/default.png');
			}
			return this.$el;
		},
		events : {
			'click #uploadBtn' : 'doUpload',
			'click #removeBtn' : 'doRemove'
		},
		doUpload : function(e){
			e.preventDefault();
	...
	var ProductRelatedImgs	= Backbone.View.extend({
		...
		doAdd : function(e){
			e.preventDefault();
			var prdId = $('.breadcrumb .active').text().trim();
			var that = this;
			...
			product.fetch({
				success : function(item, resp, opts){
					...
					var productRelatedImg = new ProductRelatedImg({model:prdImg});
					that.$el.find('#imgs').append(productRelatedImg.render());
				}
			});
		}
	});
