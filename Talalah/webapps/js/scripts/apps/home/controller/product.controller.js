...
define(function(require, exports,module){
	...
	var ProductEvents = function(){
								...
								success:function(item,resp,opt){
									...
									customer.fetch({
										success : function(item, resp, opt){
											...
											// pop up dialog for cart summary
											$('.cart_num').text(total);
											$('.cart_sum').text(sum);
											$('.noti_bubble').text(total);
											// update badge class for total cart item
											$('.alert-success').show();
					...
					customer.fetch({
						success : function(item,resp,opts){
							...
							orderTravel.save(orderTravelData, {
								...
								error : function(item,resp,opt){
									$('#MsgDialog1').modal('toggle');
									$('#MsgDialog1').find('.alert-danger').show();
								}
							});
						}
					});
		...
		this.productMerchant = {
				doLoadMore : function(that,ProductViews){
		...
		this.productMerchantCode = {
				...
				searchTravel : function(that,ProductViews){
					var page = that.pageTvl;
				...
				searchAdvTravel : function(that, ProductViews){
					that.$el.find('#products').empty();
					that.$el.find('#advTvl').modal('toggle');
					that.$el.find('#loader').show();
					that.$el.find('#loadmore').hide();
		...
		this.productSearch = {
				doSearchMore : function(page,that,ProductViews){
					that.$el.find('#loader').show();
					that.$el.find('#loadmore').hide();
					var data = that.model.toJSON();
					var key = data.key;
