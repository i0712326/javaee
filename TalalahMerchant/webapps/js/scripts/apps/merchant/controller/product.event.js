	...
	var ProductEvents = function(){
		...
		this.doUploadImg = function(that){
			...
			that.$el.find('#lefile').change(function() {
				...
				var uri = '/TalalahMerchant/merchant/product/upload/img?_csrf='+csrf;
				var postData = {
						url:uri,
						method:'post',
						contentType: false,
		...
		this.doUpdateInfo = function(that){
			...
			that.model.urlRoot = urlRoot+'?_csrf='+csrf;
			model.save(data,{
				success : function(item,resp,opt){
					setTimeout(function(){
		...
		this.doSaveProduct = function(that){
			...
			if(mcc==='4722'){
				urlRoot = '/TalalahMerchant/merchant/product/add/travel';
				type = "com.emc.app.entity.product.travel.Travel";
				productCategory = {id:'T',note:'TRAVEL'};
			}
			else{
				urlRoot = '/TalalahMerchant/merchant/product/add/item';
				type = "com.emc.app.entity.product.item.Item";
				productCategory = {id:'I',note:'ITEM'};
			}
			var rate ={ "star1": 0, "percent1": 0, 
						"star2": 0, "percent2": 0,  
						"star3": 0, "percent3": 0, 
						"star4": 0, "percent4": 0, 
						"star5": 0, "percent5": 0
			  		   };
			var model	= 	{pId : id, name : name, price : price, url : url, type : type, merchant : merchant, productCategory : productCategory, rate : rate};
			var product = 	new Model.Product();
			product.urlRoot = urlRoot+'?_csrf='+csrf;
			product.save(model,{
		...
		this.doUpdateDetail = function(that){
			...
			if( "message" in data ) delete data.message;
		...
		this.uploadRelateImg = function(that){
			...
			that.$el.find('input[type=file]').change(function() {
				...
				var uri = '/TalalahMerchant/merchant/product/upload/relate/img?_csrf='+csrf;
				var postData = {
						url:uri,
						method:'post',
						contentType: false,
					    ...
					    success:function(data){
					    	setTimeout(function(){					    		
					    		...
					    		that.$el.find('#editable').show();
					    		that.$el.find('#removeBtn').removeAttr('disabled');
					    		
					    		that.$el.find('input[name=prdId]').val(data.product.id);
					    		that.$el.find('input[name=picName]').val(data.picName);
		...
		this.removeRelateImg = function(that){
			var data = that.model.toJSON();
			var id = data.id;
			var csrf = $('input[name=_csrf]').val().trim();
			var url = '/TalalahMerchant/merchant/product/delete/relate/img?id='+id+'&_csrf='+csrf;
